...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (response.data.status === 'success') {
                localStorage.setItem('swichlocation_user', JSON.stringify(response.data.user));
                navigate('/dashboard');
            } else {
    ...
    const handleGoogleMock = () => {
        // Mocking Google login for demonstration since OAuth wasn't explicitly provided
        setError("Google login simulation requires backend setup. Using regular login.");
    };
        ...
        <div className="auth-container">
            <div className="auth-card glass-panel">
                ...
                <p style={{ textAlign: 'center', color: 'var(--text-secondary)', marginBottom: '2rem' }}>
                    Log in to SwichLocation to manage your EXIF tags.
                </p>
                ...
                <button onClick={handleGoogleMock} className="btn btn-secondary">
                ...
                <p style={{ textAlign: 'center', marginTop: '2rem', color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
                    Don't have an account? <Link to="/register" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: 500 }}>Register now</Link>
                </p>
            </div>
        </div>
    );
